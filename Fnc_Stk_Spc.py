...
from Fnc_Stk_Plt import align_yaxis, ScaledFormatter
...
def Cube_Mask_Nan(cube_fits_ifn,*args,**kwags):
	...
	for fatia in range(len(scidata1)):
		if np.all(scidata[fatia,:,:]==0.):
			scidata[fatia,:,:] = np.nan
			pass
		else:
			pass
	hdu_list.flush()
...
def Cube_Frequency_Smooth(CFS_ifn,freq_rfr,pre_res,pst_res,*args,**kwargs):
	...
	pixel_scale        = CFS_ifn_header[11]* u.km/u.s#pre_res * u.km/u.s
...
def Cube_Spatial_Extract_Circular(CSEC_ifn,X_C,Y_C,radii_px_inner,radii_as_inner,radii_px_outer,radii_as_outer,radii_px_msure,radii_as_msure,*args,**kwargs):
	...
	#mask_ms.data[mask_ms.data == 0] = np.nan
	...
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_AVG',header_comment = 'FIR Lum Average')                   for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_MED',header_comment = 'FIR Lum Median')                    for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_1SL',header_comment = 'FIR Lum 1 sgm lw lmt 15.9 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_1SH',header_comment = 'FIR Lum 1 sgm hg lmt 84.1 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_2SL',header_comment = 'FIR Lum 2 sgm lw lmt 2.30 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_2SH',header_comment = 'FIR Lum 2 sgm hg lmt 97.7 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_3SL',header_comment = 'FIR Lum 3 sgm lw lmt 0.20 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_3SH',header_comment = 'FIR Lum 3 sgm hg lmt 99.8 pct')     for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_P25',header_comment = 'FIR Lum 25 pct')                    for csec_ofns_cube in csec_ofns]
	#[Header_Copy(csec_ofns_cube,CSEC_ifn,'STF_P75',header_comment = 'FIR Lum 75 pct')                    for csec_ofns_cube in csec_ofns]
...
def Cube_fit_2D_Gaussian(Cube2bFit,*args,**kwargs):
	...
	if slc_nmb == None:
		...
		slice_nmbr = 'CSL'
		#slice_cct  = slc_nmb#(Header_Get(Cube2bFit,'FTS_CCT'))
		if dest_dir_clp != None:
	...
	if Cube2bFit_Err == None:
		...
		chi2          = np.nansum(((g_obs-g_exp)**2)/g_err,axis=None)#*(1/float(len(XAXIS)-1))
		chi2_red      = chi2/float(len(ydata)-1)

		g_obs_sgm     = g_obs#func_2D_Gaussian((x, y),X0_F   ,Y0_F   ,Header_Get(Cube2bFit,'SNR_AVG'),SIGMAX_F     ,SIGMAY_F     ,THETA_F,OFFSET_F,circular=circular)
		g_exp_sgm     = func_2D_Gaussian((x, y),X0_f2DG,Y0_f2DG,Header_Get(Cube2bFit,header_amp_guess),SIGMAX_F,SIGMAY_F,THETA_F,OFFSET_F,circular=circular)
		g_exp_sgm     = (np.asarray(g_exp_sgm)).reshape(nx, ny)
		chi2_sgm      = np.sum(((g_obs_sgm-g_exp_sgm)**2)/g_err,axis=None)#*(1/float(len(XAXIS)-1))
		chi2_red_sgm  = chi2_sgm/float(len(ydata)-1)

		g_obs_amp     = g_obs#func_2D_Gaussian((x, y),X0_F   ,Y0_F   ,A_F                            ,sigma_fwhm_2d,sigma_fwhm_2d,THETA_F,OFFSET_F,circular=circular)
		g_exp_amp     = func_2D_Gaussian((x, y),X0_f2DG,Y0_f2DG,A_F,sigma_fwhm_2d,sigma_fwhm_2d,THETA_F,OFFSET_F,circular=circular)
		g_exp_amp     = (np.asarray(g_exp_amp)).reshape(nx, ny)
		chi2_amp      = np.sum(((g_obs_amp-g_exp_amp)**2)/g_err,axis=None)#*(1/float(len(XAXIS)-1))
		chi2_red_amp  = chi2_amp/float(len(ydata)-1)
	########################CHI-SQUARE########################
	...
	DGF_vx1 = np.sqrt(2 * np.pi) * abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F * slice_sigm#  * slice_sigm
	DGF_vx2 = np.sqrt(2 * np.pi) * abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F * vl_wdt_clp#  * vl_wdt_clp
	DGF_vx3 = np.sqrt(2 * np.pi) * abs((SIGMAX_F/SIGMAX_f2DG)) * abs((SIGMAX_F/SIGMAX_f2DG)) * A_F #  #* slice_cwdt * tlt_ch_nmb
	if src_sze_fxd == True:
...
def Cube_fit_1D_Gaussian(Cube2bPlot_1D,*args, **kwargs):
	...
	if Cube2bPlot_1D_Err == None:
		...
		print
		FLUX2bPlotErr = Cube_Stat_Slice(Cube2bPlot_1D_Err,cubewdthv=cubewdthv,frq_r=frq_r,verbose=False)
